			...
			<div className={styles.authWrapper}>
				<div className="card">
					<div style={{ display: "flex", alignItems: "center", marginBottom: "20px" }}>
						<button onClick={() => router.push("/")} className="btn btn-secondary" style={{ marginRight: "15px", padding: "8px 16px" }}>
							← Terug
						</button>
					...
					<form onSubmit={handleSubmit} className={styles.formStack}>
						...
						{isLogin && (
							<div
								style={{ display: "flex", alignItems: "center", marginBottom: "10px", padding: "8px", backgroundColor: stayLoggedIn ? "#f0f9ff" : "transparent", borderRadius: "6px", border: stayLoggedIn ? "1px solid #0ea5e9" : "1px solid #e5e7eb" }}
							>
								<input type="checkbox" id="stayLoggedIn" checked={stayLoggedIn} onChange={(e) => setStayLoggedIn(e.target.checked)} style={{ marginRight: "8px", transform: "scale(1.1)" }} />
								<label htmlFor="stayLoggedIn" style={{ fontSize: "14px", cursor: "pointer", color: stayLoggedIn ? "#0369a1" : "#374151", flex: 1 }}>
									Ingelogd blijven
									<span style={{ display: "block", fontSize: "12px", color: stayLoggedIn ? "#0891b2" : "#6b7280", marginTop: "2px" }}>{stayLoggedIn ? "Sessie blijft 5 dagen actief" : "Sessie vervalt na 1 uur"}</span>
								</label>
							</div>
						)}

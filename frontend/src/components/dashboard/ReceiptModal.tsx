...
/**
 * Interface defining the properties required for the ReceiptModal component.
 */
interface ReceiptModalProps {
    /** The receipt data object to be displayed or edited. Null if no receipt is selected. */
    receipt: Receipt | null;
    /** Boolean flag to control the visibility of the modal. */
    isOpen: boolean;
    /** Callback function to close the modal. */
    onClose: () => void;
    /** Async callback function to save the updated receipt data. */
    onSave: (updatedReceipt: Receipt) => Promise<void>;
    /** Async callback function to delete the specific receipt. */
 ...
 * Manages form state, validation, and API interactions for updates and deletions.
 * @param {ReceiptModalProps} props - Component props containing receipt data and handlers.
 * @returns {JSX.Element|null} The rendered modal overlay or null if closed/empty.
 */
export const ReceiptModal: React.FC<ReceiptModalProps> = ({ receipt, isOpen, onClose, onSave, onDelete }) => {
    const [isEditing, setIsEditing] = useState(false);
    const [isLoading, setIsLoading] = useState(false);

    // Memoized initial data for viewing
    ...
    useEffect(() => {
        if (receipt && isOpen) {
            const formattedDate = receipt.purchase_date ? String(receipt.purchase_date).split("T")[0] : "";

            const formattedTime = receipt.purchase_time ? receipt.purchase_time.substring(0, 5) : receipt.purchase_date && receipt.purchase_date.includes("T") ? receipt.purchase_date.split("T")[1].substring(0, 5) : "12:00";
    ...
    /**
     * Validates and prepares the receipt data for saving.
     * Constructs the payload and calls the onSave prop.
     */
    const handleSave = async () => {
        ...
        try {
            ...
            let finalDate = receipt.purchase_date;

            const formDate = apiData.purchase_date;
            const formTime = apiData.purchase_time;

            if (formDate) {
                const timePart = formTime ? formTime : "00:00";
                finalDate = `${formDate}T${timePart}:00.000Z`;
            }

            const updatedReceipt: Receipt = {
                id: receipt.id,
                total_amount: apiData.total_amount || 0,
                purchase_date: formDate || receipt.purchase_date?.split("T")[0] || "",
                purchase_time: formTime || undefined,
            ...
            // Update the modal's data with the saved changes
            const savedData = {
                store_name: updatedReceipt.store_name,
                date: updatedReceipt.purchase_date?.split("T")[0] || "",
                time: updatedReceipt.purchase_time || "",
                total_price: updatedReceipt.total_amount,
                ...
                dangerous_metadata: updatedReceipt.dangerous_metadata || {
                    card_fingerprint: null,
                    card_network: null,
                    bank_name: null,
                    wealth_rating: null,
    ...
    const cancelEditing = () => {
        if (receipt) {
            ...
            const formattedTime = receipt.purchase_time?.substring(0, 5) || "12:00";
     ...
     * @param {string | number | null} value - The input value.

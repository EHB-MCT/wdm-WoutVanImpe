...
export function useReceiptEditor(options: UseReceiptEditorOptions = {}) {
  ...
  const updateEditableData = useCallback((field: keyof ReceiptData, value: string | number | null | ReceiptItem[] | undefined) => {
  ...
  const updateItem = useCallback((index: number, field: keyof ReceiptItem, value: string | number | null) => {
  ...
  const initializeData = useCallback((data: ReceiptData) => {
